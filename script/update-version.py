...
table = (
    ...
    , ( 'CMakeLists.txt'
        , r'set\W+bit_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( bit_lite_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+bit_lite_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( bit_lite_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\bit_lite_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'bit_lite_version = "{major}.{minor}.{patch}"\n' )
...
#        , 'version = "{major}.{minor}.{patch}"' )
    ...
    , ( 'include/nonstd/bit.hpp'
        , r'\#define\s+bit_lite_MAJOR\s+[0-9]+\s*$'
        , '#define bit_lite_MAJOR  {major}' )

    , ( 'include/nonstd/bit.hpp'
        , r'\#define\s+bit_lite_MINOR\s+[0-9]+\s*$'
        , '#define bit_lite_MINOR  {minor}' )

    , ( 'include/nonstd/bit.hpp'
        , r'\#define\s+bit_lite_PATCH\s+[0-9]+\s*$'
        , '#define bit_lite_PATCH  {patch}\n' )
)
